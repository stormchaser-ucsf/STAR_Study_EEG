function grid_sp = detect_spindles_ECoG(data_main,I,ied_times,...
    ch_names,bpFilt1,Fs)
%function grid_sp = detect_spindles_ECoG(data_main,I,ied_times,...
%    ch_names,bpFilt1,Fs)
...
for i=1:size(data_main,1)  
    ...
    disp(['Processing ch ' num2str(i)])
    
    temp1 = data_main(i,:); % raw data
    temp1 = filtfilt(bpFilt1,temp1);% filtered spindle data
    II = I;

    % ied artifacts
    art = ied_times.(ch_names{i}).ied_times;    
    art = unique(art);
    II(art) = 0;
    
    
    % get all artifact free spindle amplitudes
    %temp = ((temp1));    
    temp = abs(hilbert(temp1));% spindle amplitudes
    sp_amp = temp(logical(II));  

    % amplitude threshold
    sp_thresh1 = mean(sp_amp) + 3*std(sp_amp);
    sp_thresh2 = mean(sp_amp) + 1.0*std(sp_amp);
    
    % find all spindle peaks
    ...
    % check if the duration criteria are met (300ms to 3000ms) 
    sp_peak=[];sp_st=[];sp_end=[];
    for j=1:length(aa)
        % get the time when it went below 2nd thresh pre peak
        k = aa(j);
        ...
        k = aa(j);
        while temp(k) > sp_thresh2
            k=k+1;
        end
        stp = k-1;
        dur = stp-st;
        
        % getting number of peaks in actual spindle data 
        [peak_val1 peak_loc1] = findpeaks(temp1(st:stp));
        no_of_peaks = length(peak_val1);
        
        if dur*(1000/Fs) > 300 && dur*(1000/Fs)<3000 && no_of_peaks>=3 && st > 1000
